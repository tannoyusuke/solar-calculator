...
import { CaseStudy } from "@/data/caseStudies.en";
import { Locale } from "@/lib/i18n-config";

export default function CaseStudyClient({ dict, caseStudies, lang }: { dict: any, caseStudies: CaseStudy[], lang: Locale }) {
    const categories = [dict.all, ...Array.from(new Set(caseStudies.map((c) => c.category)))];
    const [selectedCategory, setSelectedCategory] = useState(dict.all);

    const filteredStudies = caseStudies.filter((study) =>
        selectedCategory === dict.all || study.category === selectedCategory
    );
        ...
        <div className="pt-32 pb-24 min-h-screen bg-background">
            <div className="max-w-7xl mx-auto px-6 md:px-12">
                ...
                <div className="mb-16">
                    ...
                    <h1 className="text-4xl md:text-6xl font-display font-medium tracking-widest text-white mb-8 leading-tight" dangerouslySetInnerHTML={{ __html: dict.title }} />
                    <p className="text-gray-400 font-sans tracking-wide leading-relaxed max-w-2xl text-lg whitespace-pre-line">
                        {dict.description}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {filteredStudies.map((study, i) => (
                        ...
                        >
                            {/* Image Part */}
                            <div className="relative w-full aspect-[16/10] overflow-hidden border-b border-white/5 shrink-0">
                                <Image
                                    src={study.image}
                                    alt={study.title}
                                    fill
                ...
                <div className="mt-32 text-center pt-20 border-t border-white/5">
                    <p className="text-gray-400 font-sans tracking-wide mb-8" dangerouslySetInnerHTML={{ __html: dict.ctaText }} />
                    <Link
                        href={`/${lang}/document`}
