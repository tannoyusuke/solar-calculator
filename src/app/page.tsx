...
const SolarInvestmentCalculator = () => {
  ...
  const [remainingYears, setRemainingYears] = useState<number>(12.5);
  const [mwCapacity, setMwCapacity] = useState<number>(1.6);
  const [postFitPrice, setPostFitPrice] = useState<number>(12);
  const [targetIRR, setTargetIRR] = useState<number>(5.5);

  // Calculate operating costs based on revenue
  const defaultOperatingCost = useMemo(
    () => Math.round(annualRevenue * 0.165),
    [annualRevenue]
  );
  const [operatingCost, setOperatingCost] = useState<number>(defaultOperatingCost);
  ...
  const calculations = useMemo(() => {
    try {
      ...
      const calculatePV = (cashFlow: number, year: number, irr: number): number => {
        return cashFlow / Math.pow(1 + irr / 100, year);
      };

      // Generate yearly FCF series for FIT period
      const fitPeriodFCF = Array.from(
        { length: Math.ceil(remainingYears) },
        (_, i) => {
      ...
      const fitPeriodPV = fitPeriodFCF.reduce(
        (sum, fcf, i) => sum + calculatePV(fcf, i + 1, targetIRR),
        0
      );

      const postFitPeriodPV = postFitPeriodFCF.reduce(
        (sum, fcf, i) =>
          sum + calculatePV(fcf, i + Math.ceil(remainingYears) + 1, targetIRR),
        0
      );
      ...
      const finalPriceMin = Math.round(
        irrBasedPrice * 0.6 + multipleBasedPriceMin * 0.4
      );
      const finalPriceMax = Math.round(
        irrBasedPrice * 0.6 + multipleBasedPriceMax * 0.4
      );
      ...
      const annualPaymentPerMillion =
        (1000000 * interestRate * Math.pow(1 + interestRate, loanPeriod)) /
        (Math.pow(1 + interestRate, loanPeriod) - 1);

      // FIT期間のみのDSCR計算
      const maxAnnualPayment = annualEBITDA / targetDSCR;
      const maxBorrowingAmount =
        (maxAnnualPayment / annualPaymentPerMillion) * 1000000;
      ...
      const maxBorrowingByYear = yearlyEBITDA.map(
        (ebitda) => (ebitda / targetDSCR / annualPaymentPerMillion) * 1000000
      );

      const maxBorrowingAmountWithPostFit = Math.min(...maxBorrowingByYear);

      // Calculate average and total FCF
      const averageAnnualFCF =
        annualEBITDA * (1 - (annualDegradation * remainingYears) / 2);
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <div className="mt-2 text-sm text-gray-600">
              <p>
                IRRベース価格: {formatNumber(calculations.irrBasedPrice)}円
              </p>
              <p>
                マルチプルベース価格:{' '}
                {formatNumber(calculations.multipleBasedPriceMin)}円～
                {formatNumber(calculations.multipleBasedPriceMax)}円
              </p>
            </div>
          </div>
          ...
          <div className="text-sm text-gray-600">
            ...
            <p>
              ※ 買収価格はIRRベース(60%)とマルチプルベース(40%)の加重平均
            </p>
            <p>
              ※ 卒FIT後の想定発電量：
              {formatNumber(calculations.estimatedAnnualGeneration)}
              kWh/年
            </p>
            <p>
              ※ 卒FIT後の想定売電収入：
              {formatNumber(calculations.postFitAnnualRevenue)}円/年
            </p>
            <p>
              ※ DSCR基準借入可能額（FIT期間のみ）：
              {formatNumber(calculations.maxBorrowingAmount)}円
            </p>
            <p>
              ※ DSCR基準借入可能額（卒FIT含10年）：
              {formatNumber(calculations.maxBorrowingAmountWithPostFit)}円
            </p>
          </div>
        </div>
      </CardContent>
